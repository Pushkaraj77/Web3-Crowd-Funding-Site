...
const DisplayCampaigns = ({ title, isLoading, campaigns}) => {
    ...
    return (
    <div>
      <h1 className='font-epilogue font-semibold text-title-text
      dark:text-white text-[18px] text-left'>{title} ({campaigns.length})</h1>
    ...
    <div className='flex flex-wrap mt-[20px] gap-[26px]'>
        ...
        .filter(campaign => {
            if (searchState === '') {
                return campaign;
              } else if (campaign.title.toLowerCase().includes(searchState.toLowerCase())) {
                return campaign;
              }
        })
        .map
